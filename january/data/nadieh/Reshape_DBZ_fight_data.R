...
fights <- read.csv2("Dragonball Z fights.csv", stringsAsFactors = F)
...
for(i in 1:nrow(fights)) {
  for(j in 0:13) {
    ...
    if(fights[i,10+j*2] == "") break
    
    fightPerson <- rbind(fightPerson, data.frame(id = fights$id[i], name = str_trim(fights[i,10+j*2]),
                                                 state = str_trim(fights[i,11+j*2]), subSaga = str_trim(fights$subSaga[i]),
                                                 stringsAsFactors = F))
  }#for j
}#for i
rm(i,j)
...
fights <- fights[,c("id","subSaga","charactersGood","charactersBad","info","anime","extra")]
