...
function createStraightSkyMapLayout(opts, focus_map, w, w_increase, h, map_id) {
    ...
    if(map_id !== "header") setBorderColor(map_id, chosen_culture)
    ...
    if(map_id === "constellations") {
        d3.selectAll(".culture-info-wrapper")
            .on("click", function() {
                ...
                //Change the color of the top and bottom border
                setBorderColor("constellations", chosen_culture)
...
//Set the top and bottom border color of the chosen culture sky map
function setBorderColor(map_id, culture) {
    d3.select(`#canvas-${map_id}`)
        .style("border-top", `10px solid ${cultures[culture].color}`)
        .style("border-bottom", `10px solid ${cultures[culture].color}`)
